...
import './css/mode_button.css'

const ModeBtn = () => {

    const [mode, setMode] = useState('')

    Store.useListener('start_mode', setMode)

    const switchMode = () => {
        const theme = mode === 'dark' ? 'light' : 'dark'
        localStorage.setItem('Mode', theme)
        Store.setListener('mode', theme)
        setMode(theme)
    }

    return (
        <div className={`main_icon mode_button ${mode}`} onClick={() => switchMode()}>
            
            <svg className='moon_icon' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff" stroke-width="0.00024000000000000003">
